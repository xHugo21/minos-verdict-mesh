(function initFileAnalyzer(root) {
  ...
  async function analyzeFiles(files) {
    ...
    console.log(
      `[MinosVerdictMeshExtension] Analyzing ${files.length} files: ${fileNames} (${totalSize} bytes total)`,
    );
    ...
    try {
      ...
      if (result.error) {
        console.warn(
          `[MinosVerdictMeshExtension] Backend validation error: ${result.error}`,
        );
      }
      ...
      console.log(
        `[MinosVerdictMeshExtension] Analysis complete for ${files.length} files:`,
        {
      ...
      console.error(
        `[MinosVerdictMeshExtension] Error analyzing ${files.length} files:`,
        error,
      );
      throw error;
    }
