...
async def _validate_and_save_uploaded_file(
    ...
    try:
        debug_log(
            f"[MinosVerdictMeshBackend] Saved file to {tmp_path} ({file_size} bytes)"
        )
        ...
        try:
            ...
            debug_log(f"[MinosVerdictMeshBackend] Detected MIME: {detected_mime}")
...
async def detect(
    ...
    try:
        ...
        try:
            if tmp_paths:
                debug_log(
                    f"[MinosVerdictMeshBackend] Processing text + {len(tmp_paths)} file(s)"
                )
                ...
                debug_log("[MinosVerdictMeshBackend] Processing text only:", text)
                result = await GuardOrchestrator(GUARD_CONFIG).run(
                    text=text,
                    min_block_level=block_level,
                )
            ...
            debug_log(
                "[MinosVerdictMeshBackend] Detected fields:",
